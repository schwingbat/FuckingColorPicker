...
const c = {
  ...
  warn(...messages) {
    if (DEBUG) console.warn(...messages);
  },
  error(...messages) {
    if (DEBUG) console.error(...messages);
  }
};
...
(function() {
  let messages = fs
    .readFileSync(path.join(__dirname, "loadmessages.txt"), "utf-8")
    .split("\n");
...
function handleStreamLoad(stream) {
  el.renderer.srcObject = stream;
  el.renderer.onloadedmetadata = e => {
...
function applyOptions(options) {
  ...
  document
    .querySelector("#orbit-open")
    .classList[options.showOrbit ? "remove" : "add"]("disabled");
}
...
window.addEventListener("mousedown", e => {
  if (e.target.classList.contains("copy-button")) {
    let kind = e.target.parentNode.parentNode.querySelector(".label")
      .textContent;
    let val = e.target.parentNode.parentNode.querySelector(".color")
      .textContent;
